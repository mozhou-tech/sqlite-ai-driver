...
3. 存储时自动在原始文件名前添加6位随机字符串前缀（格式：`随机字符串_原始文件名`）
4. 提供文件的存入、删除、查看基本信息、获取绝对路径等功能
5. 使用 SQLite 数据库存储文件元数据
6. 自动计算文件 MD5 值
7. 支持 MIME 类型和扩展元数据
...
1. 文件ID格式为 `YYYYMMDD/随机字符串_原始文件名`，其中日期目录自动创建，随机字符串为6位小写字母和数字的组合
2. 存储文件时，系统会自动在原始文件名前添加6位随机字符串前缀（例如：`a3f9k2_test.txt`），避免文件名冲突
